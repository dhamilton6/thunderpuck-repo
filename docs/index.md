...
```
heroku create thunderpuck-prod --remote prod && \
    heroku addons:create newrelic:wayne --app thunderpuck-prod && \
    heroku addons:create heroku-postgresql:hobby-dev --app thunderpuck-prod && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="thunderpuck-prod" \
        --app thunderpuck-prod
```

Creating the qa sever:

```
heroku create `thunderpuck-qa --remote qa && \
    heroku addons:create newrelic:wayne && \
    heroku addons:create heroku-postgresql:hobby-dev && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="thunderpuck-qa" \
```
